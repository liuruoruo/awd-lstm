...
class WDLSTMCell(RNNCell):
	'''Vanilla LSTM implemented with same initializations as BN-LSTM'''
	def __init__(self, num_units, training):
		self.num_units = num_units
		self.training = training
		self.mask = tf.Variable(tf.ones([self.num_units, 4 * self.num_units]), name="mask", trainable=False)
	...
	def set_mask(self, keep_prob):
		mask_vec = tf.ones([self.num_units, 1])
		mask_vec = tf.nn.dropout(mask_vec, keep_prob)
		return tf.assign(self.mask, tf.tile(mask_vec, [1, 4 * self.num_units]))
	...
	def __call__(self, x, state, scope=None):
		with tf.variable_scope(scope or type(self).__name__):
			...
			#if self.training:
			#	masked_W_hh = W_hh * self.mask 
			#	W_both = tf.concat([W_xh, masked_W_hh], 0)
			#else:
